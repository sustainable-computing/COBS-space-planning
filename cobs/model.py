...
class Model:
    ...
    def __init__(self,
        ...
        self.eplus_get_actuators = dict() if eplus_get_actuators is None else eplus_get_actuators
    ...
    def add_configuration(self,
        ...
        for key in values:
            if key.upper() == "NAME":
                try:
                    self.delete_configuration(idf_header_name, values[key])
                except KeyError:
                    pass
                break
        idf_header_name = idf_header_name.upper()
    ...
    def _step_callback(self):
        ...
        if self.eplus_get_actuators:
            actuator_logs = dict()
            for (component_type, control_type, actuator_key), actuator_name in self.eplus_get_actuators.items():
                handle = self.api.exchange.get_actuator_handle(component_type, control_type, actuator_key)
                if handle != -1:
                    actuator_logs[actuator_name] = self.api.exchange.get_actuator_value(handle)
            with open(f"logging/logging-{current_state['timestep']}.pkl", "wb") as logging_file:
                pickle.dump(actuator_logs, logging_file)
    ...
    def get_link_zones(self):
        ...
        for wall in walls:
            ...
            elif wall.Outside_Boundary_Condition_Object and wall.Outside_Boundary_Condition_Object in wall_to_zone:
